...
namespace APICore.API.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        /// <summary>
        /// Register a new organization and its first admin user in a single request.
        /// Either both are created or neither (atomic). Use this instead of calling
        /// create-organization and register separately to avoid inconsistent state.
        /// </summary>
        [HttpPost]
        [Route("register-with-organization")]
        [AllowAnonymous]
        [ProducesResponseType((int)HttpStatusCode.Created)]
        [ProducesResponseType(typeof(ApiResponse), (int)HttpStatusCode.BadRequest)]
        public async Task<IActionResult> RegisterWithOrganization([FromBody] RegisterWithOrganizationRequest request)
        {
            await _accountService.SignUpWithOrganizationAsync(request);
            return Created("", true);
        }
        ...
        /// <summary>
        /// Login with Google. Send the Google ID token obtained from the client (e.g. Google Identity Services).
        /// If the email already exists, the account is linked and the user is signed in. If not, a new user is created.
        /// </summary>
        /// <param name="request">Request with IdToken from Google Sign-In.</param>
        [AllowAnonymous]
