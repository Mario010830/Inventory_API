...
namespace APICore.Services
{
    public interface IAccountService
    {
        ...
        Task SignUpWithOrganizationAsync(RegisterWithOrganizationRequest request);

        Task<(User user, string accessToken, string refreshToken)> LoginAsync(LoginRequest loginRequest);

        Task<(User user, string accessToken, string refreshToken)> LoginWithGoogleAsync(GoogleLoginRequest request);
